...
def add_product(product: ProductCreate, db: Session = Depends(get_session)):
...
def read_cart(db: Session = Depends(get_session)):
...
def create_cart_item(cart_item: CartItemCreate, db: Session = Depends(get_session)):
    try:
        new_item = add_to_cart(db, cart_item)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def edit_cart_item(
    ...
    try:
        updated_item = update_cart_item(db, cart_item_id, cart_item)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def remove_cart_item(cart_item_id: int, db: Session = Depends(get_session)):
    deleted = delete_cart_item(db, cart_item_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Cart item not found")
    return Response(status_code=status.HTTP_204_NO_CONTENT)
